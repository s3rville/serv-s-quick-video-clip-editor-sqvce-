...
def norm_xf(xf):
    ...
    (yuv420 chroma), sizes within 16..16384 so nothing collapses to zero or explodes."""
    cw, ch, px, py, pw, ph = xf
    ev = lambda v: max(16, min(16384, int(round(v / 2.0)) * 2))
    return (ev(cw), ev(ch), int(round(px / 2.0)) * 2, int(round(py / 2.0)) * 2, ev(pw), ev(ph))


def xf_filter(xf):
    """ffmpeg -vf chain: scale the picture, pad with BLACK to the union of picture+canvas, then
    crop to the canvas. Handles crop, shrink (black borders) and grow (overflow cropped)."""
    cw, ch, px, py, pw, ph = xf
    ux0, uy0, ux1, uy1 = min(0, px), min(0, py), max(cw, px + pw), max(ch, py + ph)
    f = f"scale={pw}:{ph},"
    if (ux1 - ux0, uy1 - uy0) != (pw, ph):
        f += f"pad={ux1 - ux0}:{uy1 - uy0}:{px - ux0}:{py - uy0}:color=black,"
...
class CropOverlay(QWidget):
    ...
    def mouseMoveEvent(self, e):
        ...
        d = e.position() - p0
        st = self.stage
        b = st.canvas_rect() if st.tool == "crop" else QRectF(self.rect())
        r = QRectF(r0)
        if h == "move":
            r.translate(d)
...
class VideoView(QGraphicsView):
    ...
    def set_layout(self, pre=None, pic=None, rot=0, mirror=False):
        self._lay = None if pre is None else (pre, pic, rot, mirror)
        self._relayout()
    ...
    def _relayout(self):
        ...
        if self._lay is None:
            ...
            return
        (cw, ch), (px, py, pw, ph), rot, mir = self._lay
        it.setAspectRatioMode(Qt.AspectRatioMode.IgnoreAspectRatio)
        it.setSize(QSizeF(max(1.0, pw), max(1.0, ph)))
        it.setPos(0, 0)
        t = QTransform()
        t.translate(w / 2.0, h / 2.0)           # (applied bottom-up) picture -> canvas -> mirror -> rotate
        t.rotate(rot)
...
class VideoStage(QWidget):
    ...
    committed = Signal(str, QRectF)      # tool, selection in stage pixels (emitted on OK)
    dragStarted = Signal()
    ...
    def __init__(self, video):
        ...
        self._rc = QRectF()
        self._k = 1.0
        # fixed bottom-right action bar: [W] x [H] [Cancel] [OK]
        ...
        for b in (self.b_cancel, self.b_ok, self.b_swap, self.b_rot):
            b.setFocusPolicy(Qt.FocusPolicy.NoFocus)
            b.setAutoDefault(False)
    ...
    def _apply_fields(self):
        ...
        r = self._cur_units()
        if self.tool == "crop":
            w, h = max(16, min(w, cw)), max(16, min(h, ch))
            x, y = max(0, min(r.x(), cw - w)), max(0, min(r.y(), ch - h))
        else:
    ...
    def _ok(self):
        if self.ed_w.hasFocus() or self.ed_h.hasFocus():
            self._apply_fields()
        if self.pend is not None:
            pre = self._t().inverted()[0].mapRect(self.pend)     # displayed -> pre-rotation units
            k, rc = self._k, self._rc
            self.committed.emit(self.tool, QRectF(rc.x() + pre.x() * k, rc.y() + pre.y() * k,
                                                  pre.width() * k, pre.height() * k))
        self.okClicked.emit()

    def _layout_pic(self, p):
    ...
    def relayout(self):
        ...
        if xf is None or (self.xf is None and not editing and not fx_on):     # untouched clip: fill like before
            ...
            k = min(av.width() / dw, av.height() / dh)
            rc = QRectF(av.center().x() - dw * k / 2, av.center().y() - dh * k / 2, dw * k, dh * k).toRect()
            self.canvas.setGeometry(rc)
            self.vclip.setGeometry(rc)
            self._k = rc.width() / dw
            self._rc = QRectF(rc)
            self.video.setGeometry(0, 0, rc.width(), rc.height())
...
class Timeline(QWidget):
    ...
    def set_zoom(self, pps, anchor_x=None):
        pps = max(1.0, min(800.0, pps))
        if anchor_x is None:
            anchor_x = self.HW + (self.width() - self.HW) / 2
        t = self.xt(anchor_x)
        self.pps = pps
        self.scroll_x = max(0.0, t * pps - (anchor_x - self.HW))
        self._update_bar()
        self.zoomChanged.emit(pps)
        self.update()
    ...
    def zoom_fit(self):
        ...
        self.scroll_x = 0.0
        self.set_zoom(view / total, anchor_x=self.HW)
...
class MainWindow(QMainWindow):
    ...
    def build_topbar(self):
        ...
        self.hb_combo.currentIndexChanged.connect(
            lambda _=0: (self._hb_settings.setValue("hb_sel", self.hb_combo.currentText()),))
    ...
    def build_timeline_panel(self):
        ...
        self.zoom_slider.valueChanged.connect(lambda v: self.tl.set_zoom(2 * 400 ** (v / 100)))
        self.tl.zoomChanged.connect(self.sync_zoom)
        fit = QPushButton("Fit")
        fit.setFixedWidth(46)
        fit.clicked.connect(self.tl.zoom_fit)
